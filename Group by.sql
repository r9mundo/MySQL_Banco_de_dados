...
-- COMANDO GROUP BY ----> MUITO USADO COM FUNÇÃO DE AGREGAÇÃO 

select * from aluno;

select sum(idade), min(idade), max(idade), avg(idade) from aluno;

select count(id_aluno),idade
from aluno
group by idade;

select count(id_aluno), sexo
from aluno
where idade > 20
group by sexo
having sexo = 'm';
...
select count(id_autor), sexo
from autor
group by sexo;

select count(id_autor), nacionalidade
from autor
...
from autor
group by nacionalidade
having nacionalidade = 'britânica';
